...
class PatchMatch(object):
    def __init__(self, image_src, image_dest, image_src2, image_dest2, patch_size, initial_NNF=None, device=None):
        ...
        self.image_src2 = m(image_src2)[0].detach().clone().to(self.device)  # c x h x w
        self.image_dest2 = m(image_dest2)[0].detach().clone().to(self.device)  # c x h x w
    ...
    @staticmethod
    def calc_dist_parallel(img_src,img_dst,img_src2,img_dst2,patch_size,NNF,src_h,src_w, device):
        ...
        for offset_i in reversed(range(patch_size)):
            for offset_j in reversed(range(patch_size)):
                ...
                '''src_NNF_shifted=src_shifted[:,nnf_padded[:,:, 0]+patch_size//2, nnf_padded[:,:, 1]+patch_size//2].view(src_shifted.size())'''
        ...
        #distance img_src2 and img_dst2
        for offset_i in reversed(range(patch_size)):
            for offset_j in reversed(range(patch_size)):
                
                #shift dest_image and then slice it to NND size
                dest_shifted=torch.roll(img_dst2,shifts=(offset_i-patch_size//2,offset_j-patch_size//2),dims=(1,2))
                dest_shifted=dest_shifted[:,nnf_padded[:,:, 0]+patch_size//2, nnf_padded[:,:, 1]+patch_size//2].view(dest_shifted.size())
                dest_shifted=dest_shifted[:,patch_size//2:dest_shifted.size(1)-patch_size//2,patch_size//2:dest_shifted.size(2)-patch_size//2]
                
                
                #shift image_src, apply padded NNF, and then slice it to NND size
                src_shifted=torch.roll(img_src2,shifts=(offset_i-patch_size//2,offset_j-patch_size//2),dims=(1,2))
                '''src_NNF_shifted=src_shifted[:,nnf_padded[:,:, 0]+patch_size//2, nnf_padded[:,:, 1]+patch_size//2].view(src_shifted.size())'''
                src_NNF_shifted=src_shifted
                src_NNF_shifted=src_NNF_shifted[:,patch_size//2:src_NNF_shifted.size(1)-patch_size//2,patch_size//2:src_NNF_shifted.size(2)-patch_size//2]
                arr=arr+torch.sum((dest_shifted-src_NNF_shifted).pow(2),dim=0)
    ...
    def init_dist(self,):            
            self.NND=self.calc_dist_parallel(self.image_src,self.image_dest,self.image_src2,self.image_dest2,self.patch_size,self.NNF,self.src_h,self.src_w, self.device)
        ...
        for jump in jumps_arr:
            ...
            for offset_i in ([-jump,0,jump]):
                for offset_j in ([-jump,0,jump]):
                    ...
                    nnd_res=self.calc_dist_parallel(self.image_src,self.image_dest,self.image_src2,self.image_dest2,self.patch_size,nnf_shifted,self.src_h,self.src_w,self.device)
                    ...
                    #print(cat_nnd_arrs.size())
                    min_arr,argmin_arr=torch.min(cat_nnd_arrs,dim=0) #Maybe applying argmin will be faster
                    


                    #cat_nnf_arrs=torch.stack([nnf_shifted[c1:c2,c3:c4,:],self.NNF[c1:c2,c3:c4,:]])
    ...
    def random_search(self,rand_search_radius, alpha, printFlag=False, alternative_prob_domain_fix=True):
        ...
        for radius in radius_arr:
            ...
            if(self.NNF.shape[0]-2*radius-2>0 and alternative_prob_domain_fix):
              ...
              x_offsets=torch.cat([-(torch.arange(radius+1)-radius),torch.zeros(self.NNF.shape[0]-2*radius-2, dtype=int),torch.arange(radius+1)]).unsqueeze(0).to(self.device)
            if(self.NNF.shape[1]-2*radius-2>0 and alternative_prob_domain_fix):
              #if radius is less than half of the image
              y_offsets=torch.cat([-(torch.arange(radius+1)-radius),torch.zeros(self.NNF.shape[1]-2*radius-2, dtype=int),torch.arange(radius+1)]).to(self.device)
    ...
    def upsample_NNF(self, factor=2):
        ...
        new_nnf[:, :, 0] =self.NNF[:,:,0][mesh]*factor+x.repeat(self.NNF.size(0)).unsqueeze(0)
        new_nnf[:, :, 1] =self.NNF[:,:,1][mesh]*factor+torch.transpose(y.repeat(self.NNF.size(1)).unsqueeze(0),0,1)
    ...
    def reconstruct_avg(self, patch_size=5):
        ...
        m = torch.nn.ReflectionPad2d(patch_size // 2).to(self.device)
        avg_dest=m(avg_window(self.image_dest.clone().unsqueeze(0)))[0]
    ...
    def reconstruct_without_avg(self):
        img = torch.zeros((3, self.NNF.size(0), self.NNF.size(1)))
        dest_i=self.NNF[:, :, 0] + self.patch_size // 2
        dest_j=self.NNF[:, :, 1] + self.patch_size // 2
        mesh=[dest_i,dest_j]
        img[0,:]= self.image_dest[0][mesh]
        img[1,:]= self.image_dest[1][mesh]
        img[2,:]= self.image_dest[2][mesh]
        return img.permute(1,2,0).cpu().numpy()


    def run(self, num_iters=10, rand_search_radius=200, alpha=0.5,jump_radius=8, allow_diagonals=True):  
        for iteration in range(1, num_iters + 1):  
              self.propagate(jump_radius, allow_diagonals=allow_diagonals) #based on jumping flooding         
              self.random_search(rand_search_radius, alpha)  
